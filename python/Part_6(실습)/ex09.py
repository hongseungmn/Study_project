...
#비어있는 세트를 만드는 방법은 {}를 사용하지 아니하고 set()를 호출하면 된다.
#세트에 값을 추가하기(add() 사용)
#세트는 모든 요소들을 해싱기법을 이용하여 저장하고 관리한다. 그래서 요소들은 
#해싱가능(Hashtable)하여야 한다.
#해싱이란 각각의 객체에 식별할 수 있는 해쉬코드를 부여하여 객체를 테이블에 저장을 하는 것이다.
set4 = set()
set4.add("봄")
set4.add("여름")
set4.add("가을")
...
#세트는 요소가 해싱이 가능하려면 해쉬 코드를 가져야 한다.
