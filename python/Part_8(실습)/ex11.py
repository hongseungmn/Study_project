...
#매개 변수를 주지 않았을 때는 전체 문자열을 대상으로 값과 빈도수를 출력
#매개변수를 주었을 때는 전체 문자열을 대상으로 빈도수를 순서가 높은 순으로 n개 만큼 출력을 해준다.
...
#Counter 모듈의 교집합 및 합집합을 출력하는 기능
c1 = Counter("abcdefg")
c2 = Counter("abcd가나다라")
...
#덧셈과 뺄셈은 Counter 모듈과 작용하지만 나머지 연산자(*, /, %)는 지원하지 않는다.
