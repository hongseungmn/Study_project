#Counter 모듈은 시퀀스 자료형의 데이터의 값의 개수를 딕셔너리 형태로 반환해주는 자료구조이다.
...
#Counter 객체를 만드는데 인자값으로 리스트 형을 주었다.
count = Counter(text)
print(type(count))
print(count)
#시퀀스 자료형에서 어떤 특수문자의 개수를 알고 싶을 때도 사용하면 된다.
print(count["h"])
print("--------------------------------------------------")
#Counter() 객체를 만들 때, 값 = 개수와 같은 형태로 입력도 가능하다.
...
counts =Counter(a=3,b=5)
print(counts)
print(sorted(counts.elements()))
print("--------------------------------------------------")
#아래 문자열은 소문자로 다 바뀌고 단어별로 리스트가 반환되어진다.

text = ("abcdeffg repeat count! you can do it nice to meet you. A Count" + \
"telehpone!!!!").lower().split()
