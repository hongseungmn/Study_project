...
# 스택, 튜플, 딕셔너리) 좀 더 확장하여 제작된 파이썬의 내장 모듈이다.
# 1)deque(데크, double-ended queue) 모듈은 스택과 큐를 모두
# 지원하는 모듈이다. 양방향으로 데이터를 입출력 할 수 있는 자료구조이다.
...
# deque에 있는 요소들을 삭제하기
#print(deque_list.pop(0)) #deque에서는 pop(0)이라는 것이 지원이 안된다.
#deque에서 스택도 지원하는 모듈이므로 pop() 사용하면 늦게 들어간 데이터부터
#삭제됨을 알 수가 있다.
...
print("--------------------------------")
#appnedleft() 메소드 사용 : 새로운 요소들을 왼쪽부터 입력되게 하여
